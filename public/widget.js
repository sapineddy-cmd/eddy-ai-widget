(function () {
  ...
  #edy-panel{position:fixed;left:50%;transform:translateX(-50%);bottom:${BOTTOM+SIZE+8}px;width:${PANEL.w}px;height:${PANEL.h}px;max-width:92vw;
  ...
  @media (max-width:768px){ #edy-panel{ width:92vw; height:45vh; } }`;
  ...
  function toggle(){ p.style.display = (p.style.display==='flex' ? 'none' : 'flex'); if(p.style.display==='flex') txt.focus(); }
  ...
  // Envoi (Enter + bouton) → appelle ton API
  async function sendMsg(){
    const msg=(txt.value||'').trim(); if(!msg) return;
    add('me', esc(msg)); txt.value='';
    try{
      const r = await fetch(ORIGIN + '/api/reply', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ message: msg }) });
